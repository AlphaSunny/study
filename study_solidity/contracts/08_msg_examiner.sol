...
contract msgExaminer {
    ...
    function msgExaminer() public payable 
    {
	...
	function getContractCreationGas() public view returns (uint) 	// returned 732117 for me. Must be the gas expended 
    {										              		// the creation of this contract. msg.gas should be msg.gasExpended	
    	return contract_creation_gas;
    }
	
    function getContractCreationValue() public view returns (uint) // returns the original endowment of the contract
    {										              		// set at creation time with "value: <someweivalue>"	
    	return contract_creation_value;                         // this is now the "balance" of the contract
    }
    
    function getContractCreationTxGasprice() public view returns (uint) // returned 50000000000 for me. Must be the gasprice 	
    {											     				 // the sender is willing to pay. msg.gasPrice should be msg.gasLimit
    	return contract_creation_tx_gasprice;
    }
    
    function getContractCreationTxOrigin() public view returns (address) // returned my coinbase address.
    ...
    function sendOneEtherToHome() public payable returns (bool success)         	
    {						
    	msg_gas_before_creator_send = msg.gas;			// save msg values
    	msg_data_before_creator_send = msg.data;	
    	msg_value_before_creator_send = msg.value;			  
    ...
    function getMsgGasBefore() public view returns (uint)          
    {						
    	return msg_gas_before_creator_send;							  
    }
    
    function getMsgGasAfter() public view returns (uint)         
    {						
    	return msg_gas_after_creator_send;							  
    }
    
   
    function getMsgValueBefore() public view returns (uint)          
    {						
    	return msg_value_before_creator_send;							  
    }
    
    function getMsgValueAfter() public view returns (uint)         
    {						
