...
if($cfm_code != $_SESSION['authcode'])
    exit_error('139', 'there is something wrong with the cfm code');


$bind_name = 'email';
$pass = "password_login";
//判断该用户是否存在
$row = get_ba_id_by_variable($bind_name, $email);
$ba_id = $row['ba_id'];

if(empty($row['ba_id'])||$row['bind_flag'] == '9'){
    exit_error('112', 'this email is not registered');
}
if($row['bind_flag'] == '2')
    exit_error('118', 'this email is rejected');
if($row['bind_flag'] == '3')
    exit_error('137', 'this email is refused');
