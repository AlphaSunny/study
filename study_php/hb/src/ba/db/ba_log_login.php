 ...
 * @return int
 */
function get_ba_lgn_log($ba_id)
{
    $db = new DB_COM();
    $sql = "SELECT * FROM ba_log_login WHERE ba_id = '{$ba_id}'";
    $db -> query($sql);
...
{
    $db = new DB_COM();
    $sql = "SELECT * FROM ba_log_login WHERE ba_id = '{$us_id}' ORDER BY utime DESC limit {$offset},{$limit}";
    $db->query($sql);
    $rows = $db->fetchAll();
    return $rows;
}