...
def worker(msg, starttime):
    print("任务执行的时刻", time.time(), "传达的消息是", msg, '任务建立时刻', starttime)


# 创建一个调度器示例
# 第一参数是获取时间的函数，第二个参数是延时函数
print ('----------  两个简单的例子  -------------')
print ('程序启动时刻：', time.time())
s = sched.scheduler(time.time, time.sleep)
s.enter(1, 1, worker, ('hello', time.time()))
s.enter(3, 2, worker, ('world', time.time()))
s.run()  # 这一个 s.run() 启动上面的两个任务
print ('睡眠２秒前时刻：', time.time())
time.sleep(2)
print ('睡眠２秒结束时刻：', time.time())
'''
...
print ('程序结束时刻', time.time())
'''

timer = threading.Timer(10, worker('hello world', time.time()))
