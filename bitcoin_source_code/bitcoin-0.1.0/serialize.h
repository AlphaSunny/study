...
//seriliaze.h实现了所有的序列化函数。CDataStream类是数据序列化的核心结构
...
//
/*
CDataStream拥有一个字符类容器用来存放序列化之后的数据。它结合一个容器类型和一个流（stream）界面以处理数据。
*/
class CDataStream
{
protected:
    /*
    vch存有序列化后的数据。它是一个拥有自定义内存分配器的字符容器类型。该内存分配器将由该容器的实现在需要分配/释放内存时调用。
    该内存分配器会在向操作系统释放内存前清空内存中的数据以防止本机的其他进程访问此数据，从而保证数据存储的安全性。
    该内存分配器的实现在此不进行讨论，读者可于serialize.h自行查找。
    */
...
public:
    ...
    /*
    nType的取值为SER_NETWORK，SER_DISK，SER_GETHASH，SER_SKIPSIG，SER_BLOCKHEADERONLY之一，
    其作用为通知CDataStream进行具体某种序列化操作。这5个符号被定义在一个枚举类型enum里。
    每个符号均为一个int类型（4字节），并且其值为2的次方。
    */
    int nType;
    int nVersion;
...
否则，调用memcpy(pch, &vch[nReadPos], nSize)复制nSize个字符，从vch的nReadPos位置开始，
到由pch指向的一段预先分配的内存。接着从nReadPos向前移至下一个起始位置nReadPosNext（第22行）。
该实现表明
1）当一段数据被从流中读取之后，该段数据无法被再次读取；
2）nReadPos是第一个有效数据的读取位置。
