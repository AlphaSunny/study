...
//函数模板，将16进制的数转换成为无符号整形
template<typename T>
string HexNumStr(const T itbegin, const T itend, bool f0x=true)
{
    //unsigned char为无符号整形，八位，0~255
    const unsigned char* pbegin = (const unsigned char*)&itbegin[0];
    const unsigned char* pend = pbegin + (itend - itbegin) * sizeof(itbegin[0]);
    //f0x如果是true, str= "0x", 如果不是就是"",默认是true
    string str = (f0x ? "0x" : "");
    for (const unsigned char* p = pend-1; p >= pbegin; p--)
        //%02X表示如果没超过两位，前面加上0，如果超过两位，没有影响
        str += strprintf("%02X", *p);
    return str;
}
...
//生成哈希值的方法，也是模板，该函数对对目标函数执行两次SHA256()方法并返回结果。
//SHA256()的申明可在openssl/sha.h中找到
//在比特币源码中的hash都是做两次哈希256
