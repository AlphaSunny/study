...
bool AddKey(const CKey& key)
{
    ...
    {
        mapKeys[key.GetPubKey()] = key.GetPrivKey();              //建立公钥和私钥的一一对应关系
        mapPubKeys[Hash160(key.GetPubKey())] = key.GetPubKey();    //建立公钥的哈希和公钥本身的对应关系
    }
...
{
    CKey key;
    key.MakeNewKey();                      //生成一个新的CKey对象
    if (!AddKey(key))                         //通过addKey将新建的key添加至全局映射map mapKeys和全局映射map mapPubKeys 和钱包数据库wallet.dat
        throw runtime_error("GenerateNewKey() : AddKey failed\n");           //处理异常
    return key.GetPubKey();                    //返回公共钥匙
	//该公钥为未压缩的格式，属于OPENSSL标准格式之一。在得到公钥之后，比特客户端会将该公钥传递至PubKeyToAddress()并调用Hash160toAdress（）方法生成
	//地址。最后返回的Base58编码字符串值便是一个新生成的比特币地址。Base58不包括易混淆的如 0 (零), O (大写字母O), I (大写的字母i) and l (小写的字母L) 
}
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // txNew.vout[0].nValue       = 5000,000,000    单位是satoshi
...
3.wtx是一个新的Tx实例。
4.nFeeRequiredRet是一笔用来支付交易费的输出交易，在该方法执行完成后获得
*/
{
    nFeeRequiredRet = 0;
    CRITICAL_BLOCK(cs_main)
    {
        ...
        CRITICAL_BLOCK(cs_mapWallet)
        {
            ...
            loop
            {
                ...
                if (nValue < 0)
                    return false;
                int64 nValueOut = nValue; //定义一个本地变量nValueOut = nValue来保存转账的金额。
                nValue += nFee;   //将nValue和交易费相加来得到包含转账费的nValue

                // Choose coins to use
                set<CWalletTx*> setCoins;
                if (!SelectCoins(nValue, setCoins))     //得到一系列币，并放入setCoins。setCoins指包含你本人支付地址
														//的交易，即你本人所拥有的币，这些交易将成为wtxNew的来源交易
                    return false;
                int64 nValueIn = 0;
                foreach(CWalletTx* pcoin, setCoins)
                    nValueIn += pcoin->GetCredit();        //nValue指你拥有的所有的币

                // Fill vout[0] to the payee
                wtxNew.vout.push_back(CTxOut(nValueOut, scriptPubKey));     //添加一笔输出交易至wtxNew。该笔输出将支付给
																			//<收款人地址160位哈希>（包含在scriptPubKey里面）数量为的币。

                // Fill vout[1] back to self with any change
                if (nValueIn > nValue)             //如果需要找零，添加另一笔输出交易至wtxNew并将零钱发回本人
                {
                    /// todo: for privacy, should randomize the order of outputs,
                    //        would also have to use a new key for the change.
                    // Use the same key as one of the coins
                    vector<unsigned char> vchPubKey;  //如果txFrom是你本人创建的，则脚本B会被添加至CreateTransaction()中txFrom的某一笔输出交易。在这里，第44行位于CreateTransaction()中的公钥vchPubKey是你本人的公钥。
                    CTransaction& txFirst = *(*setCoins.begin());        //从setCoin中获取第一笔交易txFirst，依次检查txFirst.vout中的交易
																			//是否属于本人，如果是，从该笔交易中提取公钥，并放入本地的vchPubKey
...
/*1.scriptPubKey包含脚本代码OP_DUP OP_HASH160 <收款人地址160位哈希> OP_EQUALVERIFY OP_CHECKSIG。
  ...
  3.wtxNew是一个CWalletTx类的本地变量。该变量目前的值为空，之后会包含若干CMerkleTX类对象。
...
*/
{
    ...
    {
        int64 nFeeRequired;
        if (!CreateTransaction(scriptPubKey, nValue, wtxNew, nFeeRequired))     //首先建立一笔新的交易
        {
            string strError;
        ...
        if (!CommitTransactionSpent(wtxNew))        //尝试将这笔交易提交至数据库
        {
        ...
        // Broadcast
        if (!wtxNew.AcceptTransaction())         //如果该交易提交成功，将其广播至其他peer节点
        {
        ...
        wtxNew.RelayWalletTransaction();                       //广播至其他节点
    }
    MainFrameRepaint();
    return true;
}
